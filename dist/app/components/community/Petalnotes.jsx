...
function Petalnotes({ className }) {
  const [petalnotes, setPetalnotes] = (0, react_1.useState)([]);
  ...
  const categories = [
    { id: 'all', name: 'All Notes', icon: lucide_react_1.BookOpen },
    { id: 'guides', name: 'Guides', icon: lucide_react_1.BookOpen },
    { id: 'tutorials', name: 'Tutorials', icon: lucide_react_1.Edit },
    { id: 'analysis', name: 'Analysis', icon: lucide_react_1.Search },
    { id: 'reviews', name: 'Reviews', icon: lucide_react_1.Heart },
  ];
  if (loading) {
    return (
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredPetalnotes.map(note => (
          <card_1.Card key={note.id} className="cursor-pointer transition-shadow hover:shadow-lg">
            <card_1.CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-pink-100 dark:bg-pink-900">
                    <lucide_react_1.User className="h-4 w-4 text-pink-600 dark:text-pink-400" />
                  </div>
                  <div>
                ...
                <badge_1.Badge variant={note.isPublic ? 'default' : 'secondary'}>
            ...
            <card_1.CardContent className="pt-0">
              ...
              <div className="mb-3 flex flex-wrap gap-1">
                {note.tags.slice(0, 3).map(tag => (
                  <badge_1.Badge key={tag} variant="secondary" className="text-xs">
                    <lucide_react_1.Tag className="mr-1 h-3 w-3" />
                    {tag}
                  </badge_1.Badge>
                ))}
                {note.tags.length > 3 && (
              ...
              {note.collaborators.length > 0 && (
                <div className="mb-3 flex items-center gap-1">
                  <lucide_react_1.Users className="h-3 w-3 text-gray-400" />
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    <lucide_react_1.Heart
                      className={`h-4 w-4 ${note.isLiked ? 'fill-current' : ''}`}
                    />
                    {note.likes}
                  </button>
                  <button
      ...
      {filteredPetalnotes.length === 0 && (
        <div className="py-12 text-center">
          <lucide_react_1.BookOpen className="mx-auto mb-4 h-12 w-12 text-gray-400" />
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <card_1.Card className="mx-4 w-full max-w-2xl">
            <card_1.CardHeader>
              <card_1.CardTitle>Create New Petalnote</card_1.CardTitle>
            </card_1.CardHeader>
            <card_1.CardContent className="space-y-4">
              <input_1.Input placeholder="Note title..." />
              <textarea
              ...
              <div className="flex gap-2">
                <button_1.Button onClick={() => setShowCreateModal(false)} variant="outline">
                  Cancel
                </button_1.Button>
                <button_1.Button>Create Note</button_1.Button>
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </div>
