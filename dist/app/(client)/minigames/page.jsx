...
exports.default = MiniGamesPage;
const Header_1 = __importDefault(require('../../components/Header'));
const react_1 = require('react');
const image_1 = __importDefault(require('next/image'));
const PetalCollector_1 = __importDefault(require('../../components/PetalCollector'));
const Achievements_1 = __importDefault(require('../../components/Achievements'));
const TradeCraft_1 = __importDefault(require('../../components/TradeCraft'));
const framer_motion_1 = require('framer-motion');
const GameCubeBoot_1 = __importDefault(require('../../components/GameCubeBoot'));
const MENU_ITEMS = [
  {
    id: 'hub',
    label: 'Hub',
...
function MiniGamesPage() {
  const [bootComplete, setBootComplete] = (0, react_1.useState)(false);
  const [showCube, setShowCube] = (0, react_1.useState)(false);
  const [bootDone, setBootDone] = (0, react_1.useState)(false);
  const [showFlash, setShowFlash] = (0, react_1.useState)(false);
  const [activeFace, setActiveFace] = (0, react_1.useState)('hub');
  const [isRotating, setIsRotating] = (0, react_1.useState)(false);
  const [showParticles, setShowParticles] = (0, react_1.useState)(false);
  const [particles, setParticles] = (0, react_1.useState)([]);
  const audioRef = (0, react_1.useRef)(null);
  const hoverSoundRef = (0, react_1.useRef)(null);
  ...
  return (
    <main className="min-h-screen bg-black">
      ...
      <framer_motion_1.AnimatePresence mode="wait">
        {!bootComplete && <GameCubeBoot_1.default onBootComplete={handleBootComplete} />}
          ...
          <div className="cube-scene perspective-1000 relative flex flex-col items-center">
            ...
            <div className="face-menu absolute left-1/2 top-1/2 z-10 grid -translate-x-1/2 -translate-y-1/2 grid-cols-2 gap-6">
              {MENU_ITEMS.map(f => (
                <button
                  key={f.id}
                  className={`menu-btn bg-gradient-to-br ${f.color} flex flex-col items-center rounded-xl border-2 border-white/20 px-8 py-6 shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-pink-400 ${activeFace === f.id ? 'scale-110 ring-4 ring-pink-400' : ''}`}
