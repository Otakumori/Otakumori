...
const FriendList = () => {
  const [activeTab, setActiveTab] = (0, react_1.useState)('friends');
  const [searchQuery, setSearchQuery] = (0, react_1.useState)('');
  const { friends, friendRequests, acceptFriendRequest, rejectFriendRequest, toggleFavorite } = (0,
  friendStore_1.useFriendStore)();
  ...
  return (
    <div className="mx-auto w-full max-w-md rounded-lg bg-white/10 p-4 shadow-lg backdrop-blur-lg">
      <div className="mb-4 flex space-x-4">
        <button
          className={`flex-1 rounded-lg px-4 py-2 transition-colors ${activeTab === 'friends' ? 'bg-pink-500 text-white' : 'bg-white/20 text-white'}`}
        ...
        <button
          className={`flex-1 rounded-lg px-4 py-2 transition-colors ${activeTab === 'requests' ? 'bg-pink-500 text-white' : 'bg-white/20 text-white'}`}
