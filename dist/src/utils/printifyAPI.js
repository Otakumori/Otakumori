...
exports.publishProduct = exports.fetchProductDetails = exports.fetchProducts = void 0;
...
const fetchWithRetry = async (url, options, retries = MAX_RETRIES) => {
  try {
    const response = await (0, axios_1.default)({
      ...
      headers: {
        ...options.headers,
        Authorization: `Bearer ${env_1.env.PRINTIFY_API_KEY}`,
...
const fetchProducts = async () => {
  try {
    return await fetchWithRetry(`${BASE_URL}/shops/${env_1.env.PRINTIFY_SHOP_ID}/products.json`, {
      method: 'GET',
    });
  } catch (error) {
...
const fetchProductDetails = async productId => {
  try {
    return await fetchWithRetry(
      `${BASE_URL}/shops/${env_1.env.PRINTIFY_SHOP_ID}/products/${productId}.json`,
      {
        method: 'GET',
      }
    );
...
const publishProduct = async productId => {
  try {
    return await fetchWithRetry(
      `${BASE_URL}/shops/${env_1.env.PRINTIFY_SHOP_ID}/products/${productId}/publish.json`,
      {
        method: 'POST',
      }
    );
