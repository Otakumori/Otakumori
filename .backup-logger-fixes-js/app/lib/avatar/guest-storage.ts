...
function cleanupExpiredCharacters(): void {
  ...
  ids.forEach((id) => {
    try {
      ...
      logger.warn(`Failed to check character ${id}:`, error);
...
export function loadGuestCharacter(id: string): GuestCharacter | null {
  ...
  try {
    ...
    logger.warn(`Failed to load guest character ${id}:`, error);
  }
...
export function deleteGuestCharacter(id: string): boolean {
  ...
  try {
    ...
    logger.warn(`Failed to delete guest character ${id}:`, error);
    return false;
  }
}
...
export function updateGuestCharacter(
  ...
  try {
    ...
    logger.warn(`Failed to update guest character ${id}:`, error);
    return null;
  }
}
