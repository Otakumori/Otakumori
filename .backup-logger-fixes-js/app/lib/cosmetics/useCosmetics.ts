...
function loadUnlockedIds(): string[] {
  ...
  try {
    ...
    logger.warn('Failed to load unlocked cosmetics:', error);
  }
...
function saveUnlockedIds(ids: string[]): void {
  ...
  try {
    ...
    logger.warn('Failed to save unlocked cosmetics:', error);
  }
}
...
function loadHudSkin(): HudSkinId {
  ...
  try {
    ...
    logger.warn('Failed to load HUD skin:', error);
  }
...
function saveHudSkin(skinId: HudSkinId): void {
  ...
  try {
    ...
    logger.warn('Failed to save HUD skin:', error);
  }
}
    ...
    async (skinId: HudSkinId) => {
      ...
      if (isAuthenticated) {
        try {
          ...
          logger.error('Failed to sync HUD skin to backend:', error);
