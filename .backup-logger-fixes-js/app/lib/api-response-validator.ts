...
export function validateApiResponse<T>(response: unknown, schema: z.ZodSchema<T>): T | null {
  try {
    ...
    logger.warn('[API Response Validator] Validation failed:', {
    ...
    logger.error('[API Response Validator] Unexpected error:', error);
    return null;
  }
}
...
export function validateApiEnvelope<T>(
  ...
  if (!envelopeCheck.success) {
    logger.warn('[API Response Validator] Not a valid API envelope:', envelopeCheck.error);
    return null;
  }
  ...
  if (!envelope.ok) {
    ...
    logger.warn('[API Response Validator] API returned error:', {
  ...
  if (!dataValidation.success) {
    logger.warn('[API Response Validator] Data validation failed:', {
