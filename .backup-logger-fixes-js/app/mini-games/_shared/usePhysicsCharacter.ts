...
export function usePhysicsCharacter(
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.error('Failed to initialize PhysicsCharacterRenderer:', error);
      setIsReady(false);
    }
  ...
  const render = useCallback(
    (x: number, y: number, facing: 'left' | 'right' = 'right') => {
      ...
      try {
        ...
        logger.error('Error rendering physics character:', error);
      }
    },
    [isReady],
  );
  ...
  const update = useCallback(
    (deltaTime: number, velocity: { x: number; y: number }, position: { x: number; y: number }) => {
      ...
      try {
        ...
        logger.error('Error updating physics character:', error);
      }
    },
  ...
  const applyImpact = useCallback(
    (force: { x: number; y: number }, part: string = 'chest') => {
      ...
      try {
        ...
        logger.error('Error applying impact:', error);
      }
    },
    [isReady, physicsEnabled],
  );
