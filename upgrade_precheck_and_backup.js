...
async function main() {
  try {
    ...
    // 5. Get table counts
    await runQuery(
      'SELECT schemaname, relname, n_live_tup FROM pg_stat_user_tables ORDER BY n_live_tup DESC LIMIT 50;',
      '05_table_counts',
    );
    ...
    try {
      ...
      if (dbUrl) {
        ...
        try {
          ...
          console.warn(
            '   Details:',
            pgDumpError instanceof Error ? pgDumpError.message : pgDumpError,
          );
        }
      } else {
