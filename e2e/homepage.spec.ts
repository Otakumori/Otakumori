...
test.describe('Homepage', () => {
  ...
  test('should show fallback content when live data is disabled', async ({ page }) => {
    ...
    const fallbackMessages = page.locator('text=currently undergoing maintenance, text=No mini-games available, text=No soapstone messages found');
  ...
  test('should prevent duplicate soapstone submissions', async ({ page }) => {
    ...
    if (await composer.isVisible()) {
      ...
      await expect(page.locator('text=Please wait before submitting a similar message')).toBeVisible();
    }
  });
