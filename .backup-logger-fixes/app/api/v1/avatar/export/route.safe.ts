...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.warn('Avatar export requested:', {
      format,
      quality,
    ...
    logger.error('Avatar export error:', error);
...
async function generateGLBExport(config: any, quality: string) {
  ...
  logger.warn('Generating GLB export with quality:', quality, 'config keys:', Object.keys(config));
...
async function generateFBXExport(config: any, quality: string) {
  ...
  logger.warn('Generating FBX export with quality:', quality);
  const qualityMultiplier = quality === 'high' ? 2 : quality === 'low' ? 0.5 : 1;
  return {
...
async function generateOBJExport(config: any, quality: string) {
  ...
  logger.warn('Generating OBJ export with quality:', quality);
  const qualityMultiplier = quality === 'high' ? 2 : quality === 'low' ? 0.5 : 1;
  return {
...
async function generatePNGExport(config: any, quality: string) {
  ...
  logger.warn('Generating PNG export with quality:', quality);
  const qualityMultiplier = quality === 'high' ? 2 : quality === 'low' ? 0.5 : 1;
  return {
...
async function generateJPGExport(config: any, quality: string) {
  ...
  logger.warn('Generating JPG export with quality:', quality);
  const qualityMultiplier = quality === 'high' ? 2 : quality === 'low' ? 0.5 : 1;
  return {
...
async function generateSVGExport(config: any) {
  ...
  logger.warn('Generating SVG export, config gender:', config.gender || 'female');
  return {
