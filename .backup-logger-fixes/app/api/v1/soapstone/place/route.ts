...
async function handler(request: NextRequest) {
  ...
  try {
    ...
    logger.error('Soapstone placement error:', error);
...
async function checkUserRateLimits(userId: string) {
  ...
  logger.warn('Checking rate limits for user:', userId, { oneHourAgo, oneDayAgo });
...
async function checkForDuplicates(userId: string, message: string, page: string) {
  ...
  logger.warn('Checking for duplicates:', { userId, messagelength: message.length, page });
...
async function checkLocationCooldown(userId: string, page: string, section?: string) {
  ...
  logger.warn('Checking location cooldown:', { userId, page, section: section || 'none' });
...
async function getSoapstoneCount(userId: string): Promise<number> {
  ...
  logger.warn('Getting soapstone count for user:', userId);
