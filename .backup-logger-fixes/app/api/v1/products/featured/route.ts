  ...
  try {
    ...
    if (catalogResult) {
      ...
      if (!validated.success) {
        logger.error('[API] Response validation failed:', validated.error);
    ...
    if (forcePrintify) {
      try {
        ...
        if (printifyResult && printifyResult.data.products.length > 0) {
          ...
          if (!validated.success) {
            logger.error('[API] Printify response validation failed:', validated.error);
            return NextResponse.json(
              createApiSuccess({ products: [], pagination: undefined }, requestId),
              { status: 200 },
            );
        ...
        logger.warn('[API] Printify fetch failed:', error);
    ...
    logger.error('[API] Featured products endpoint error:', error);
    return NextResponse.json(
