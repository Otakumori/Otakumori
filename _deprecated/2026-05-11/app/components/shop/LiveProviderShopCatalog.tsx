...
function dedupeProducts(products: CatalogProduct[]) {
  ...
  for (const product of products) {
    const key = [
      ...
      String(product.priceRange?.min ?? product.priceCents ?? product.price ?? ''),
      (product.image ?? product.images?.[0] ?? '').trim().toLowerCase(),
    ].join('::');
...
export default function LiveProviderShopCatalog() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch('/api/v1/catalog?limit=40', {
        ...
        if (!cancelled) {
          setProducts(dedupeProducts(loaded.filter(isLiveProduct)));
        }
      } catch (err) {
        if (!cancelled) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {visibleProducts.map((product) => {
        ...
        const summary = stripHtml(product.description || '').replace(/\s+/g, ' ').trim().slice(0, 180);
        return (
          <article key={product.id} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-lg">
            ...
            <div className="space-y-3 p-5">
              ...
              <div className="inline-flex items-center justify-center rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm text-zinc-300">
                Details coming next
              </div>
            </div>
          </article>
