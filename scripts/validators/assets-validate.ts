...
function validateAssets() {
  ...
  if (bubbleGame) {
    ...
    // Check theme fields
    const themeFields = ["themeAccent", "themeOverlayBg", "themeCreditsText", "themeCreditsUrl", "themePledgeLabel", "themePledgeUrl", "showCreditsInPause"];
    ...
    if (bubbleGame.themeCreditsText) {
      const hasMatchingCredit = credits.some(credit => 
  ...
  for (const [gameKey, gameRoles] of Object.entries(roles)) {
    ...
    const hasThemeFields = ["themeCreditsText", "themeCreditsUrl", "themePledgeLabel", "themePledgeUrl"].some(field => gameRoles[field]);
    ...
    for (const [role, url] of Object.entries(gameRoles)) {
      if (typeof url === "string" && !url.startsWith("/assets/") && url.includes(".")) {
        ...
        if (manifestPath && !manifestPath.startsWith("/assets/")) {
          console.error(`  ❌ Asset for role ${role} in ${gameKey} must live under /public/assets/* (got: ${url})`);
          hasErrors = true;
        }
      }
    }
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === "string") {
        ...
        if (!value.startsWith("/assets/")) {
          console.error(`  ❌ Asset for ${currentPath} must live under /public/assets/* (got: ${value})`);
          hasErrors = true;
        }
