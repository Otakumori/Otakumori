...
async function main() {
  ...
  if (!avatar) {
    console.error("Usage: npm run gen:pack <avatar> [-- --workflow ./comfy/workflow.json --seed 123]");
    console.error("\nExamples:");
  ...
  if (!workflow) {
    ...
    console.error("Usage: npm run gen:pack <avatar> -- --workflow ./comfy/ui-workflow.json [--avatars-workflow ./comfy/char-workflow.json]");
    process.exit(1);
  }
  ...
  for (const className of classes) {
    ...
    try {
      const cmd = `npm run gen:asset ${className} -- --avatar ${avatar} ${baseArgs} ${seedArgs} ${ditherArgs.join(" ")}`.trim();
      console.log(`  → ${cmd}`);
