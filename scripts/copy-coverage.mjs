...
function scan(file) {
  ...
  lines.forEach((ln, i) => {
    ...
    ln.replace(/\b(placeholder|alt|aria-label|title)\s*=\s*["'`]([^"'`]{1,200})["'`]/g, (_, prop, val) => {
      const txt = val.trim();
      const matchKey = flat[txt];
...
// summarize
const wired = items.filter(i => i.type === "wired" || i.type === "wiredText" || i.type === "wiredProp");
...
for (const r of raw) {
  ...
  if (!seen.has(k)) { seen.add(k); uniqRaw.push(r); }
}

// write JSON
fs.mkdirSync("reports", { recursive: true });
fs.writeFileSync(OUT_JSON, JSON.stringify({ totals: { wired: wired.length, raw: uniqRaw.length }, wired, raw: uniqRaw }, null, 2));

// write MD checklist
const toMD = (arr) => arr.map(r => `- [ ] \`${r.text}\`  \n  ↳ **${r.file}:${r.line}**`).join("\n");
