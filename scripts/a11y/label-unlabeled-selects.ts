...
(async () => {
  ...
  for (const sf of project.getSourceFiles()) {
    ...
    const selects = sf.getDescendantsOfKind(SyntaxKind.JsxOpeningElement)
    ...
    for (const open of selects) {
      ...
      if (id) {
        const hasLabel = sf.getDescendantsOfKind(SyntaxKind.JsxOpeningElement).some(o =>
