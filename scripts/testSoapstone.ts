...
async function testSoapstone() {
  ...
  templates.forEach(template => {
    ...
    template.slots.forEach(slot => {
      ...
      console.log(`     Position ${slot.position}: [${categories}] ${slot.optional ? "(optional)" : ""}`);
    });
  });
  ...
  const examples = [
    ...
    { template: "combo_tip_subject_direction", tip: "be wary of", subject: "trap", direction: "ahead" },
  ...
  const humorPhrases = await prisma.phrase.findMany({
    where: { 
      OR: [
        { category: PhraseCategory.HUMOR },
        { category: PhraseCategory.MEME }
      ]
    },
    take: 10,
