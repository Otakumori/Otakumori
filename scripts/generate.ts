...
function clamp8(x: number) { return x < 0 ? 0 : x > 255 ? 255 : x | 0; }
...
function ditherToPaletteRGBA(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      buf[i] = neu.r; buf[i+1] = neu.g; buf[i+2] = neu.b;
...
async function main() {
  ...
  if (!targetOrPreset) {
    console.error("Usage: npm run gen:asset -- <class|presetId> [--avatar <name> --game <game> --workflow <path> --seed 123 --count 4]");
    console.error("\nExamples:");
  ...
  const ditherModeCli =
    ditherFlag === "fs" ? "floyd-steinberg" :
    ditherFlag === "atkinson" ? "atkinson" : undefined;
  ...
  const classKey = (["ui", "textures", "icons", "avatars", "sfx"] as ClassKey[]).includes(targetOrPreset as any)
  ...
  if (classKey) {
    ...
    console.log(`🎯 Generating ${classKey} for avatar "${avatar}" (${vibe} vibe) → preset: ${presetId}`);
  } else {
  ...
  const ditherCfg = disableDitherCli ? null : {
    mode: (ditherModeCli as "floyd-steinberg" | "atkinson") ?? preset.dither?.mode ?? "floyd-steinberg",
    strength: ditherStrengthCli !== undefined
      ? parseFloat(String(ditherStrengthCli))
      : (preset.dither?.strength ?? 1.0),
    alphaThreshold: alphaThresholdCli !== undefined
      ? parseInt(String(alphaThresholdCli))
      : (preset.dither?.alphaThreshold ?? 24)
  };

  if (ditherCfg) {
    console.log(`🎨 Dithering: ${ditherCfg.mode} (strength: ${ditherCfg.strength}, alpha: ${ditherCfg.alphaThreshold})`);
  }
  ...
  const names: string[] = (preset.names && preset.names.length === count)
    ? preset.names
  ...
  await fsp.access(workflowPath).catch(() => { throw new Error(`Workflow not found at ${workflowPath}`); });
  ...
  const patchTexts = (wf: any, pos: string, neg: string) => {
    ...
    for (const id of nodes) {
      ...
      if (node.class_type === "CLIPTextEncode") {
        ...
        if (!posPatched && /positive/i.test(title)) {
          node.inputs = node.inputs || {}; node.inputs.text = pos; posPatched = true; continue;
        }
        if (!negPatched && /negative/i.test(title)) {
          node.inputs = node.inputs || {}; node.inputs.text = neg; negPatched = true; continue;
        }
      }
    }
    ...
    if (!posPatched || !negPatched) {
      ...
      if (!posPatched && clips[0]) { workflow[clips[0]].inputs = workflow[clips[0]].inputs || {}; workflow[clips[0]].inputs.text = pos; }
      if (!negPatched && clips[1]) { workflow[clips[1]].inputs = workflow[clips[1]].inputs || {}; workflow[clips[1]].inputs.text = neg; }
    }
  };
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (ditherCfg && preset.palette?.length) {
      ...
      ditherToPaletteRGBA(
        arr, tmp.info.width, tmp.info.height,
        preset.palette,
        ditherCfg.mode,
        ditherCfg.strength,
    ...
    const file = path.join(PUBLIC_DIR, "assets", "gen", presetId.replace(/\./g, "/"), `${name}.${hash.slice(0,8)}.png`);
