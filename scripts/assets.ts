...
function extFromUrl(u: string) { try { const pathname = new URL(u).pathname; return path.extname(pathname) || ""; } catch { return ""; } }
...
async function main() {
  ensureDir(OUT_DIR); ensureDir(PUBLIC_DIR);
  ...
  ensureDir(baseDirAbs); ensureDir(gamesDirAbs);
  ...
  for (const item of cfg.items) {
    ...
    if (type === "zip") {
      ...
      if (item.extractToGames) {
        ...
        let entry = "";
        for (const name of entryHtml) { const candidate = path.join(destFolder, name); if (fs.existsSync(candidate)) { entry = candidate; break; } }
        const publicEntry = entry ? "/" + path.relative(PUBLIC_DIR, entry).replace(/\\/g, "/") : "/games/" + slug + "/index.html";
        manifest[item.id] = { id: item.id, type: "game", src: publicEntry, file: path.relative(ROOT, destFolder), license: item.license || undefined, bytes, sha256: "" };
        continue;
      }

      const destAbs = path.join(baseDirAbs, item.dest || "");
      ensureDir(destAbs);
      zip.getEntries().forEach((entry: any) => { if (entry.isDirectory) return; const rawName = path.basename(entry.entryName); const safeName = rawName.replace(/[^a-zA-Z0-9._-]/g, "-"); const outPath = path.join(destAbs, safeName); ensureDir(path.dirname(outPath)); fs.writeFileSync(outPath, entry.getData()); });
      await fsp.unlink(tmpPath);
      const publicPath = "/" + path.relative(PUBLIC_DIR, destAbs).replace(/\\/g, "/");
      manifest[item.id] = { id: item.id, type: "zip", src: publicPath, file: path.relative(ROOT, destAbs), license: item.license || undefined, bytes, sha256: "" };
      continue;
    }
    ...
    if (item.checksum && item.checksum.length > 0 && item.checksum.toLowerCase() !== actualHash) { await fsp.unlink(tmpPath); throw new Error(`Checksum mismatch for ${item.id}. Expected ${item.checksum}, got ${actualHash}`); }
    ...
    manifest[item.id] = { id: item.id, type, src: publicPath, file: path.relative(ROOT, finalAbs), license: item.license || undefined, bytes, sha256: actualHash };
  }

  const entries = Object.entries(manifest).map(([k,v]) => `  ${JSON.stringify(k)}: ${JSON.stringify(v)},`).join("\n");
  const tsOut = `// AUTO‑GENERATED by scripts/assets.ts — do not edit by hand\n`+
...
`export const getGame = (slugOrId: string): string | null => {\n  const direct = ASSETS[slugOrId]?.src;\n  if (direct) return direct;\n  const found = Object.values(ASSETS).find(r => r.type === 'game' && (r as any).file.endsWith('/' + slugOrId));\n  return found ? (found as any).src : null;\n};\n`;
  ...
  const keys = Object.keys(manifest).map(k => JSON.stringify(k)).join(" | ");
...
main().catch((err)=>{ console.error("✖ assets.ts failed:\n", err); process.exit(1); });
