...
function ensureImport(root) {
  const hasImport =
    ...
    root.find(jsc.VariableDeclaration, {
      declarations: [{ init: { callee: { name: "require" }, arguments: [{ value: "next/image" }] } }],
    }).size() > 0;
...
module.exports = function transformer(file, api) {
  ...
  imgs.forEach(path => {
    ...
    const newAttrs = attrs.map(a => {
      if (a.type === "JSXAttribute" && a.name.name === "src" && a.value && a.value.type === "Literal") {
        // Keep literal
        return a;
      }
      return a;
    ...
    path.replace(
      j.jsxElement(newOpening, newClosing, path.node.children)
    );
    changed = true;
  });
...
module.exports.postProcess = function() {
  if (skipped.length) {
    ...
    console.log("TIP: Wrap the image in a sized container and switch to <Image fill /> manually for these.\n");
  }
};
