...
const fixes = [
  // Fix RetroSoundVisualizer
  {
    ...
    replacements: [['Object.keys(playingSounds)', 'Object.keys(_playingSounds)']],
  },

  // Fix PetalPhysicsDemo
  {
    file: 'app/components/demos/PetalPhysicsDemo.tsx',
    replacements: [['wind.direction', 'wind._direction']],
  },
  ...
  // Fix AdvancedPetalSystem
  {
    ...
    replacements: [
      ...
      [
        'const _angle = Math.atan2(petal.y, petal.x);',
        'const angle = Math.atan2(petal.y, petal.x);',
      ],
    ],
  },
  ...
  // Fix session-tracker
  {
    ...
    replacements: [
      ...
      [
        ...
        'const session = await this.getSession(_sessionId);\n    if (!session || session.synced)',
      ],
