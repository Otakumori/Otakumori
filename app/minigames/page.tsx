...
export default function MiniGamesPage() {
  ...
  return (
    <main className="min-h-screen bg-black">
      ...
      {!bootDone && (
        <div id="gc-boot" className="fixed inset-0 bg-white z-[9999] flex items-center justify-center">
          <div className="gc-logo" style={{
          ...
          <div className="gc-scanlines absolute inset-0 pointer-events-none" style={{
      ...
      {showFlash && (
        <div id="transition-flash" className="fixed inset-0 bg-[#FF2AB8] z-[9998] animate-flash-pink pointer-events-none" />
      )}
      <style>{`
      ...
      {bootDone && (
        <section className="min-h-screen flex flex-col items-center justify-center bg-black relative overflow-hidden">
          ...
          {showParticles && (
            <div className="particles absolute inset-0 pointer-events-none">
              {particles.map((particle) => (
                <div
                  key={particle.id}
                  className="particle absolute w-2 h-2 bg-pink-300 rounded-full"
                  style={{
                    left: `${particle.x}%`,
                    top: `${particle.y}%`,
          ...
          <div className="cube-scene relative flex flex-col items-center perspective-1000">
            <div className={`cube-container transition-all duration-1000 ${isRotating ? 'rotate-y-180' : ''}`}>
              <Image 
                src="/assets/otakucube.png" 
                alt="OtakuCube" 
                width={340} 
                height={340} 
                className="rounded-2xl shadow-2xl border-4 border-pink-400 transform-gpu hover:scale-105 transition-transform duration-300"
              />
            </div>
            <div className="face-menu absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 grid grid-cols-2 gap-6 z-10">
              {MENU_ITEMS.map(f => (
                <button 
                  key={f.id}
                  className={`menu-btn bg-gradient-to-br ${f.color} border-2 border-white/20 rounded-xl px-8 py-6 flex flex-col items-center shadow-lg hover:scale-110 hover:shadow-pink-400 transition-all duration-300 ${
                    activeFace === f.id ? 'ring-4 ring-pink-400 scale-110' : ''
                  }`}
                  onClick={() => handleFaceChange(f.id)}
                  onMouseEnter={handleMenuHover}
                >
                  <span className="label text-lg font-bold text-white">{f.label}</span>
                  <span className="text-sm text-white/80 mt-2">{f.description}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {activeFace === 'miniGames' && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] z-20">
              <PetalCollector />
            </div>
          )}
          ...
          {activeFace === 'achievements' && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl z-20">
              <Achievements />
            </div>
          )}
