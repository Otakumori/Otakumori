    ...
    async (searchQuery: string, searchOffset = 0, reset = false) => {
      ...
      try {
        ...
        if (reset) {
          ...
          setOffset(payload.data.results.length >= PAGE_SIZE ? PAGE_SIZE : payload.data.results.length);
        } else {
  ...
  return (
    <div className={cn("space-y-6", className)}>
          ...
          {results.map((result, index) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className={cn("rounded-lg p-2", getResultTypeColor(result.type))}>{getResultIcon(result.type)}</div>

                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-center gap-2">
                    <h3 className="truncate font-medium text-gray-900">{result.title}</h3>
                    <span className={cn("rounded-full px-2 py-1 text-xs", getResultTypeColor(result.type))}>{result.type}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                    ...
                    {typeof result.relevanceScore === "number" && <span>Relevance: {Math.round(result.relevanceScore)}%</span>}
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      {isLoading && results.length === 0 && (
        <div className="py-12 text-center">
          <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-b-2 border-pink-500" aria-label="Loading" />
