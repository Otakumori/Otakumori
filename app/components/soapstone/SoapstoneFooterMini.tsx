...
export default function SoapstoneFooterMini() {
  ...
  const submit = async () => {
    ...
    if (!t) return;
    setBusy(true);
    const res = await fetch("/api/soapstone", { method: "POST", body: JSON.stringify({ text: t }) });
    setBusy(false);
    if (res.ok) {
      setText("");
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === "Enter" && document.activeElement === el) submit(); };
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      />
      <button disabled={busy} onClick={submit} className="rounded-full bg-fuchsia-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-fuchsia-500">
        Post
      </button>
    </div>
  );
