...
export default function GamesGrid({ games }: GamesGridProps) {
  ...
  const getTechColor = (tech?: string) => {
    switch (tech) {
      case '2D': return 'bg-blue-500/20 text-blue-300';
      case 'WebGL': return 'bg-purple-500/20 text-purple-300';
      case 'Three.js': return 'bg-orange-500/20 text-orange-300';
      case 'PixiJS': return 'bg-pink-500/20 text-pink-300';
        ...
        <GlassPanel key={game.id} className="group overflow-hidden">
          <Link href={`/games/${game.slug}`} className="block">
            <div className="relative aspect-video w-full overflow-hidden">
              ...
              {/* Tech Badge */}
              {game.tech && (
                <div className={`absolute top-3 right-3 rounded-full px-2 py-1 text-xs font-medium ${getTechColor(game.tech)}`}>
                  {game.tech}
                </div>
              )}
            </div>
