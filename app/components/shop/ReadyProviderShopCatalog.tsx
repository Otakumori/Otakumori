...
export default function ReadyProviderShopCatalog() {
  ...
  if (loading) return <div className="text-zinc-300">Loading provider-backed storefront...</div>;
      ...
      {visibleProducts.map((product) => {
        ...
        return (
          <article key={product.id} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-lg">
            <Link href={`/shop-item/${encodeURIComponent(product.id)}`} className="block">
            ...
            <div className="space-y-3 p-5">
              ...
              <Link href={`/shop-item/${encodeURIComponent(product.id)}`} className="inline-flex items-center justify-center rounded-xl bg-pink-500/80 px-4 py-2 text-sm text-white hover:bg-pink-500 transition-colors">View details</Link>
            </div>
          </article>
        );
      })}
