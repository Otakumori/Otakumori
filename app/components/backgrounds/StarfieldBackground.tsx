 ...
 * - Otaku-mori "abyss" color palette (deep purples, near-black blues)
...
export default function StarfieldBackground({
  ...
  useEffect(() => {
    ...
    // If reduced motion, render static frame
    if (prefersReducedMotion) {
      ...
      const count = Math.max(140, Math.min(Math.round((area / 1800) * density), 1100));
      ctx.fillStyle = 'rgba(240, 225, 255, 0.6)';
      for (let i = 0; i < count; i++) {
        const x = Math.random() * w;
        const y = Math.random() * h;
        const size = Math.random() * 1.5 * DPR;
        ctx.beginPath();
    ...
    const buildGrad = () => {
      ...
      backdrop.addColorStop(0.0, '#1a0f2a'); // Deep purple
      backdrop.addColorStop(0.35, '#120b1f'); // Dark purple
      backdrop.addColorStop(1.0, '#080611'); // Near-black
    };
    buildGrad();
    ...
    const drawBG = () => {
      ...
      // Faint vignette + soft aurora glow (Otaku-mori abyss aesthetic)
      ...
      glow.addColorStop(0, 'rgba(210, 170, 255, 0.05)'); // Soft purple glow
    ...
    const drawStars = (dt: number) => {
      ...
      for (let i = 0; i < stars.length; i++) {
        ...
        const col = `rgba(240, 225, 255, ${alpha})`; // White-lavender stars
