...
export default async function AchievementsPanel() {
  ...
  return (
    <section aria-labelledby="achievements" className="rounded-2xl border border-white/10 bg-black/50 p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 id="achievements" className="text-lg font-semibold text-white">Achievements</h2>
        <div className="text-sm text-zinc-300">{earnedPoints}/{totalPoints} pts</div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {catalog.map((a: { code: string; name: string; description: string; points: number; icon?: string; title?: string; lore?: string }) => {
          ...
          return (
            <div key={a.code}
              ...
              <div className="relative h-12 w-12 overflow-hidden rounded-lg ring-1 ring-white/10">
                <Image src={a.icon || "/placeholder-achievement.png"} alt="" fill className={`object-contain ${owned ? "" : "grayscale"}`} />
              </div>
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-white">{a.title || a.name}</div>
