  ...
  return (
    <section id="blog" className="relative z-10 mx-auto mt-10 max-w-7xl px-4 md:mt-14 md:px-6">
      <div className="mb-4 flex items-end justify-between">
        <h2 className="text-xl font-semibold text-fuchsia-100 md:text-2xl">{t("cta", "newPost1")}</h2>
        <Link href="/blog" className="text-sm text-fuchsia-300 hover:text-fuchsia-200">{t("cta", "download2")}</Link>
      </div>
        ...
        {posts.map((p) => (
          <GlassPanel key={p.id} className="p-4">
            <Link href={`/blog/${p.slug}`} className="block">
              ...
              {p.excerpt ? <p className="mt-2 line-clamp-3 text-sm text-zinc-300/90">{p.excerpt}</p> : null}
            </Link>
          </GlassPanel>
        ))}
      </div>
