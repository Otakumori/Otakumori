...
export class PhysicsCharacterRenderer {
  ...
  protected updatePhysics(deltaTime: number): void {
    ...
    for (const particle of this.physicsEngine.particles) {
      ...
      const jiggleX = Math.sin(this.time * this.config.jiggleFrequency * Math.PI * 2) * this.config.jiggleAmplitude;
      const jiggleY = Math.cos(this.time * this.config.jiggleFrequency * Math.PI * 2) * this.config.jiggleAmplitude;
      ...
      const movementForce = Math.sqrt(velocityDelta.x ** 2 + velocityDelta.y ** 2) * this.config.movementSensitivity;
      ...
      for (let i = this.impacts.length - 1; i >= 0; i--) {
        ...
        const impactX = impact.part === 'chest' ? (this.chest?.centerX || 0) : impact.part === 'hips' ? (this.hips?.centerX || 0) : 0;
        const impactY = impact.part === 'chest' ? (this.chest?.centerY || 0) : impact.part === 'hips' ? (this.hips?.centerY || 0) : 0;
        const distance = Math.sqrt(
          (particle.position.x - impactX) ** 2 +
          (particle.position.y - impactY) ** 2
        );
    ...
    for (const particle of this.physicsEngine.particles) {
      ...
      particle.position.x = 2 * particle.position.x - particle.previousPosition.x + particle.acceleration.x * deltaTime * deltaTime;
      particle.position.y = 2 * particle.position.y - particle.previousPosition.y + particle.acceleration.y * deltaTime * deltaTime;
  ...
  protected renderSmoothPart(part: BodyPart, centerX: number, centerY: number, radius: number): void {
  ...
  protected renderCelShadedHead(): void {
    ...
    // Head base (skin tone)
    const skinGradient = this.ctx.createRadialGradient(headX, headY - 5, 0, headX, headY, headRadius);
