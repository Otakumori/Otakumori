...
export default function GameShell({
	...
	return (
		<div
			ref={shellRef}
			className={["relative mx-auto w-full max-w-6xl overflow-hidden rounded-2xl","mg-crt mg-tint","gc-shell-container",exiting ? "gc-exit" : "gc-enter"].join(" ")}
		...
		>
			...
			<div className="absolute inset-0">
				{theme.frameImg && (
					<div aria-hidden className="pointer-events-none absolute inset-0 opacity-45" style={{ background: `url(${theme.frameImg}) center/cover no-repeat` }} />
				)}
				...
				{/* Scene goes here */}
				<div aria-hidden={paused||!!ended} className={(paused||ended) ? "pointer-events-none" : ""}>
					{children}
				</div>
...
function Overlay({
	title, bg, accent, body, primary, secondary, footer
}:{
	title:string;
	bg:string;
	accent:string;
	...
	footer?: { creditsText?: string|null; creditsUrl?: string|null; pledgeLabel?: string|null; pledgeUrl?: string|null };
}){
	return (
		<div className="absolute inset-0 grid place-items-center" style={{ background: bg }}>
			<div className="relative rounded-2xl border px-6 py-4" style={{ borderColor: accent, background: "rgba(0,0,0,0.55)", boxShadow: `0 0 24px ${accent}33` }}>
				<div className="text-lg font-semibold" style={{ color: accent }}>{title}</div>
				{body && <div className="mt-2">{body}</div>}
				<div className="mt-3 flex gap-3">
					<button onClick={primary.onClick} className="rounded-xl border px-3 py-2 hover:opacity-90" style={{ borderColor: accent, color: accent, background: `${accent}1A` }}>
						{primary.label}
					</button>
					<button onClick={secondary.onClick} className="rounded-xl border px-3 py-2 hover:opacity-90" style={{ borderColor: accent, color: accent, background: `${accent}1A` }}>
						{secondary.label}
					</button>
				</div>
				...
				{!!footer && (footer.creditsText || footer.pledgeLabel) && (
					<div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-pink-200/80">
						{footer.creditsText && (
							footer.creditsUrl ? (
								<a href={footer.creditsUrl} target="_blank" rel="noreferrer" className="underline hover:opacity-90">
									{footer.creditsText}
								</a>
							) : <span>{footer.creditsText}</span>
						)}
						{footer.pledgeLabel && footer.pledgeUrl && (
							<a href={footer.pledgeUrl} target="_blank" rel="noreferrer" className="rounded-lg border px-2 py-1 hover:opacity-90" style={{ borderColor: accent, color: accent, background: `${accent}14` }}>
								{footer.pledgeLabel}
							</a>
						)}
					</div>
