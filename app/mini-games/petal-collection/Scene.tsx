...
export default function PetalCollection() {
  ...
  useEffect(() => {
    ...
    function pileIndex(x: number) {
      return Math.max(0, Math.min(63, Math.floor((x / W) * 64)));
    }
    ...
    function update(now: number, dt: number) {
      ...
      for (const p of petals) {
        ...
        const cx = catcher.x,
          cy = H - FLOOR_BAND - CATCHER_H;
        const halfW = catcher.w / 2,
          halfH = CATCHER_H / 2;
    ...
    function draw(_now: number) {
      ...
      if (bgImg && bgImg.complete && bgImg.naturalWidth) {
        ...
        g.addColorStop(0, '#0d0a0f');
        g.addColorStop(1, '#181322');
        ctx.fillStyle = g;
