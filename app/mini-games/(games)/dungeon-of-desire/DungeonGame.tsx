...
export default function DungeonGame({
  ...
  onStatsUpdate?: (stats: { score: number; health: number; maxHealth: number; floor: number; progress?: number }) => void;
} = {}) {
  ...
  useEffect(() => {
    if (gameState === 'playing' && onStatsUpdate) {
      const progress = floor / 10; // Assuming 10 floors max, adjust as needed
      onStatsUpdate({
        score,
  ...
  }, [gameState, score, player.health, player.maxHealth, floor, onStatsUpdate]);
  ...
  // Game constants
  const WORLD_WIDTH = canvasWidth * 4; // 4x canvas width for side-scrolling
  const GROUND_Y = canvasHeight * 0.83; // 83% of canvas height
    ...
    const gameLoop = () => {
      ...
      setCamera((prev) => {
        const targetX = Math.max(
          0,
          Math.min(WORLD_WIDTH - canvasWidth, player.x - canvasWidth / 2),
        );
      ...
      if (!roomEnemiesSpawned && enemies.length === 0) {
        ...
        roomLayout.forEach((enemyType, index) => {
          setTimeout(() => {
            const spawnX = player.x + canvasWidth / 2 + index * 150 + Math.random() * 100;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Enhanced background gradient overlay using CSS variables
      const accentColor = typeof window !== 'undefined'
        ? getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim() || '#db2777'
        : '#db2777';
      ...
      bgGradient.addColorStop(0, `${accentColor}1A`); // 10% opacity
      bgGradient.addColorStop(1, `${bgColor}4D`); // 30% opacity
  ...
  const drawDungeonBackground = (ctx: CanvasRenderingContext2D, cameraX: number) => {
    ...
    const bg1Gradient = ctx.createLinearGradient(0, 0, 0, canvasHeight);
    ...
    for (let i = -1; i < canvasWidth / 300 + 2; i++) {
  ...
  const drawHUD = (ctx: CanvasRenderingContext2D) => {
    ...
    // Floor and score
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    ctx.fillRect(canvasWidth - 220, 20, 200, 50);
    ctx.fillStyle = '#fbbf24';
    ctx.font = 'bold 16px monospace';
    ctx.fillText(`Floor: ${floor}`, canvasWidth - 210, 40);
    ctx.fillText(`Score: ${score}`, canvasWidth - 210, 62);

    // Controls hint
    ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
    ctx.fillRect(20, canvasHeight - 60, 300, 40);
    ctx.fillStyle = '#cbd5e0';
    ctx.font = '12px monospace';
    ctx.fillText('Move: A/D  Jump: W/Space  Cast: F/Enter', 25, canvasHeight - 35);
  };
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center bg-gradient-to-b from-purple-900 via-gray-900 to-black">
      ...
      <div ref={containerRef} className="w-full max-w-4xl aspect-[4/3]">
        <canvas
          ref={canvasRef}
          className="w-full h-full rounded-2xl border-2 border-purple-500/30 shadow-2xl"
          style={{
            backgroundColor: 'var(--om-bg-root, #1a1816)',
            imageRendering: 'pixelated',
          }}
          aria-label="Dungeon of Desire game area"
        />
      </div>
