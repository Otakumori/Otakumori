...
interface QuickMathProps {
  onGameEnd?: (results: { score: number; correct: number; wrong: number; maxStreak: number }) => void;
  onStatsUpdate?: (stats: { score: number; combo: number; timer: number; progress: number }) => void;
}

export default function QuickMath({ onGameEnd, onStatsUpdate }: QuickMathProps = {}) {
  ...
  }, [started, sTick, score, streak, onStatsUpdate]);
  ...
  return (
    <div className="absolute inset-0 grid grid-rows-[1fr_auto]" style={{ backgroundColor: 'var(--color-bg-base)' }}>
      {/* Top: question */}
