...
interface Props {
  ...
  onStatsUpdate?: (stats: { score: number; health: number; maxHealth: number; combo: number; wavesCleared: number; progress?: number }) => void;
}
...
export default function BeatEmUpGame({
  ...
  useEffect(() => {
    if (gameState.wave > 0 && onStatsUpdate) {
      const progress = mode === 'survival' 
        ? Math.min(1.0, (gameState.timeElapsed || 0) / 300) // 5 minutes max for survival
        : gameState.wave / 10; // Assuming 10 waves max for story/arcade
      onStatsUpdate({
        score: gameState.score,
        health: gameState.health,
        maxHealth: gameState.maxHealth,
        combo: gameState.combo,
        wavesCleared: gameState.wave,
        progress,
      });
    }
  }, [gameState.score, gameState.health, gameState.maxHealth, gameState.combo, gameState.wave, gameState.timeElapsed, mode, onStatsUpdate]);
  ...
  useEffect(() => {
    if (containerSize) {
      setCanvasWidth(containerSize.width);
      setCanvasHeight(containerSize.height);
    }
  }, [containerSize]);

  // Game dimensions (will be set dynamically from container)
  const GROUND_Y = canvasHeight * 0.67; // 67% of canvas height
  ...
  const spawnEnemy = useCallback((type: Enemy['type'] = 'grunt') => {
    const spawnX = Math.random() > 0.5 ? canvasWidth + 50 : -50;
    ...
    const newEnemy: Enemy = {
      ...
      x: spawnX,
      y: spawnY,
      vx: spawnX > canvasWidth / 2 ? -1.5 : 1.5,
      vy: 0,
      facing: spawnX > canvasWidth / 2 ? 'left' : 'right',
      state: 'walk',
  ...
  useEffect(() => {
    ...
    setCanvasWidth(cssWidth);
    setCanvasHeight(cssHeight);
  }, [containerSize]);
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      enemiesRef.current = enemiesRef.current.filter((enemy) => {
        ...
        enemy.x = Math.max(30, Math.min(canvasWidth - 30, enemy.x));
      ...
      if (enemiesRef.current.length > 0 && aliveEnemies.length === 0 && enemySpawnTimer > 1000) {
        ...
        // Wave change handled via onStatsUpdate
        enemySpawnTimer = 0;
  ...
  const render = (ctx: CanvasRenderingContext2D) => {
    ...
    // Enhanced background overlay with glow using CSS variables
    ...
    overlayGradient.addColorStop(0, `${accentColor}1A`); // 10% opacity
    overlayGradient.addColorStop(1, 'transparent');
    ctx.fillStyle = overlayGradient;
    ctx.fillRect(0, 0, canvasWidth, canvasHeight);

    // Draw ground line using CSS variables
    const borderColor = getComputedStyle(document.documentElement)
      .getPropertyValue('--om-border-soft')
      .trim() || 'rgba(229, 224, 216, 0.15)';
    ...
    // Render beat indicators using design system colors
    const petalColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-petal').trim() || '#eac8d6'
      : '#eac8d6';
    beatIndicatorsRef.current.forEach((ind) => {
      ctx.save();
      ctx.globalAlpha = ind.opacity;
      ctx.fillStyle = ind.perfect ? '#FFD700' : petalColor;
      ctx.shadowBlur = ind.perfect ? 20 : 10;
      ctx.shadowColor = ind.perfect ? '#FFD700' : petalColor;
      ctx.beginPath();
    ...
    ctx.lineWidth = 2;
    ctx.strokeRect(canvasWidth - 120, 10, 100, 40);

    // Beat perfect zone
    ctx.fillStyle = 'rgba(255, 215, 0, 0.2)';
    ctx.fillRect(canvasWidth - 120, 10, 40, 40);

    // Draw petal particles using design system petal color
  ...
  const renderEnemy = (ctx: CanvasRenderingContext2D, enemy: Enemy) => {
    ...
    // Color based on type using design system colors
    const petalColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-petal').trim() || '#eac8d6'
      : '#eac8d6';
    const errorColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-error').trim() || '#c97a7a'
      : '#c97a7a';
    const accentColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-accent').trim() || '#ebe8f7'
      : '#ebe8f7';
    
    let color = petalColor;
    if (enemy.type === 'brute') color = errorColor;
    if (enemy.type === 'ninja') color = accentColor;
    if (enemy.type === 'boss') color = errorColor;
  ...
  return (
    <div className="relative">
      ...
      <div ref={containerRef} className="w-full aspect-[4/3] max-w-4xl mx-auto">
        <canvas
          ref={canvasRef}
          className="w-full h-full rounded-2xl border border-pink-500/20 shadow-2xl"
          style={{
