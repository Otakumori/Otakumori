...
interface SamuraiSliceProps {
  onGameEnd?: (results: { score: number; hits: number; crit: number; miss: number }) => void;
  onStatsUpdate?: (stats: { score: number; combo: number; timer: number; progress: number }) => void;
}

export default function SamuraiSlice({ onGameEnd, onStatsUpdate }: SamuraiSliceProps = {}) {
  ...
  useEffect(() => {
    ...
    function draw(now: number) {
      ...
      // Update stats via callback (UI is handled by GameUI component)
      const remain = Math.max(0, endAt - now);
      const timerSeconds = Math.ceil(remain / 1000);
      const progress = 1 - (remain / ROUND_TIME);
      
      // Calculate combo (simplified - could be enhanced)
      const combo = Math.floor(hits / 10);
    ...
    async function end() {
      ...
      if (onGameEnd) {
        onGameEnd({ score, hits, crit, miss });
      }
  ...
  }, [onGameEnd, onStatsUpdate, saveOnExit, autoSave, load, avatarData, drawAvatar]);

  return (
    <div ref={hostRef} className="gc-viewport mg-crt mg-tint relative w-full h-full" style={{ backgroundColor: 'var(--color-bg-base)' }}>
