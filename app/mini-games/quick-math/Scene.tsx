...
export default function QuickMath(){
  ...
  function nextQuestion(){
    ...
    let A = rng(cfg.a)+1, B = rng(cfg.b)+1, Q = "", Ans = 0;

    if (op === "+"){ Ans = A+B; Q = `${A} + ${B}`; }
    if (op === "-"){ if (A<B) [A,B] = [B,A]; Ans = A-B; Q = `${A} - ${B}`; }
    if (op === "×"){ Ans = A*B; Q = `${A} × ${B}`; }
    if (op === "÷"){
      // force clean division: choose product then divide by one factor
      const x = rng(9)+2, y = rng(9)+2;  // 2..10 tables
      Ans = x; Q = `${x*y} ÷ ${y}`;
    }
  ...
  useEffect(()=>{
    ...
    function loop(){
      if (started && !ended){
        const now = performance.now();
        if (now >= roundEnd.current){ end(now); return; }
        ...
        if (remain < 10_000 && sTick && Math.floor(remain/1000) !== Math.floor((remain+16)/1000)) play(sTick, -24);
      }
      setTick(x=>x+1);
  ...
  function submit(){
    ...
    if (val === answer){
      ...
      if (sOK) play(sOK, -10);
      // time credit & next Q
      roundEnd.current = Math.min(roundStart.current + ROUND_MS, roundEnd.current + STREAK_BONUS_MS);
      nextQuestion();
    } else {
      miss(true);
    }
  ...
  function push(n:string){
    startIfNeeded();
    if (n === "C"){ setInput(""); return; }
    if (n === "←"){ setInput(s=>s.slice(0,-1)); return; }
    if (n === "±"){
      setInput(s=>{
        if (!s) return s;
  ...
  useEffect(()=>{
    function onKey(e:KeyboardEvent){
      if (ended) return;
      if (e.key === "Enter") { startIfNeeded(); submit(); return; }
      if (e.key === "Backspace") { startIfNeeded(); push("←"); return; }
      if (e.key === "-" || e.key === "+"){ startIfNeeded(); push("±"); return; }
      if (/[0-9]/.test(e.key)){ startIfNeeded(); push(e.key); return; }
    }
  ...
  async function end(now:number){
    ...
    (window as any).__gameEnd?.({ score, durationMs: duration, stats:{ correct, wrong, maxStreak, diff } });
  }
    ...
    <div className="absolute inset-0 grid grid-rows-[1fr_auto]">
      ...
      <div className="relative flex flex-col items-center justify-center p-4">
        ...
        <div className="mt-6 text-center">
          ...
          <div className="mt-3 text-3xl font-semibold text-white/90">
            = <span className="inline-block min-w-[6ch] rounded-lg border border-white/15 bg-white/5 px-3 py-1 text-white">{input||"\u00A0"}</span>
          </div>

          {/* Submit on desktop */}
          <div className="mt-3 hidden gap-2 sm:flex">
            <button className="rounded-lg border border-amber-300/40 px-3 py-1 text-amber-200 hover:bg-amber-300/10" onClick={()=>{ startIfNeeded(); submit(); }}>Submit</button>
            <button className="rounded-lg border border-amber-300/20 px-3 py-1 text-amber-100 hover:bg-amber-300/10" onClick={()=>push("C")}>Clear</button>
          </div>
        </div>
      </div>
      ...
      <div className="select-none border-t border-white/10 bg-black/30 p-3 backdrop-blur-sm">
        <div className="mx-auto grid max-w-xl grid-cols-4 gap-2">
          {["7","8","9","←","4","5","6","±","1","2","3","C","0","","","OK"].map((k,i)=>{
            ...
            return (
              <button
                key={i}
                onClick={()=>push(k)}
                className={`h-12 rounded-xl border text-lg ${primary
