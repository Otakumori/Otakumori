...
export default function OwnedRunesGrid() {
  const { data, error, isLoading } = useSWR<InventoryResponse>('/api/trade/inventory', (u) =>
  ...
  return (
    <div>
      <div className="mb-2 text-xs text-zinc-400">Tip: fuse 2+ runes in Trade Center to discover new ones.</div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4" data-testid="runes-grid">
        {items.map((it) => (
          <motion.div
            key={it.canonicalId}
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] text-zinc-200">×{it.quantity}</span>
            </div>
            <div className="mt-1 truncate text-xs text-zinc-300" title={it.displayName || it.canonicalId}>
              {it.displayName || it.canonicalId}
            </div>
            {it.lore && (
              <div className="mt-1 hidden text-[10px] text-zinc-400 group-hover:block" aria-live="polite">
                {it.lore}
              </div>
            )}
          </motion.div>
