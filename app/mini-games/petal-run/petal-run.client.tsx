...
export default function PetalRunClient() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/mini-games/session', {
        ...
        body: JSON.stringify({ game: 'petal-run' }),
      });
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/mini-games/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ runId, game: 'petal-run', score }),
      });
      const j = await res.json();
      if (j?.ok) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/60 p-4 text-white">
      <div className="mb-2 text-pink-200">Petal Run — Ready</div>
