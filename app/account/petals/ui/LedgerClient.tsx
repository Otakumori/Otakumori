  ...
  return (
    <div className="rounded-2xl ring-1 ring-neutral-200 bg-white">
      ...
      <div className="divide-y divide-neutral-100">
        {rows.length === 0 && (
          <div className="p-6 text-neutral-500 text-sm">No activity yet.</div>
        )}
        {rows.map(r => (
          <div key={r.id} className="p-4 flex items-center justify-between">
            ...
            <div className={`text-sm font-semibold ${r.type === "SPEND" ? "text-rose-600" : r.type === "EARN" ? "text-green-700" : "text-neutral-700"}`}>
...
function labelForReason(reason: string) {
  switch (reason) {
    case "PETAL_CLICK": return "Petal click";
    case "CRIT_DROP": return "Critical drop";
    case "SHOP_PURCHASE": return "Shop purchase";
    case "REFUND": return "Refund";
    case "ADMIN_ADJUST": return "Admin adjustment";
    case "ACHIEVEMENT": return "Achievement";
    case "BONUS_EVENT": return "Bonus event";
    default: return reason.replaceAll("_"," ").toLowerCase().replace(/(^|\s)\S/g, s => s.toUpperCase());
  }
}
