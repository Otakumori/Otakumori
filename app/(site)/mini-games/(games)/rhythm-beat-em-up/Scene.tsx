...
// Default canvas dimensions (will be overridden by container size)
...
export default function Scene({ mapUrl: _mapUrl }: SceneProps) {
  ...
  // Asset loading - use preloaded assets from cache if available
  useEffect(() => {
    const loadAssets = async () => {
      const assetsToLoad: Promise<void>[] = [];

      // Load background - check cache first
      const bgUrl = getAsset('rhythm-beat-em-up', 'bg') || getAsset('rhythm-beat-em-up', 'backgrounds.city');
      if (bgUrl) {
        const cached = gameAssetCache.getElement(bgUrl);
        if (cached && cached instanceof HTMLImageElement) {
          // Use preloaded asset
          backgroundImgRef.current = cached;
        } else {
          // Load if not cached
      ...
      // Load player sprite (fallback) - check cache first
      const playerSpriteUrl = getAsset('rhythm-beat-em-up', 'sprites.player');
      if (playerSpriteUrl) {
        const cached = gameAssetCache.getElement(playerSpriteUrl);
        if (cached && cached instanceof HTMLImageElement) {
          // Use preloaded asset
          playerSpriteRef.current = cached;
        } else {
          // Load if not cached
      ...
      // Load enemy sprite - check cache first
      const enemySpriteUrl = getAsset('rhythm-beat-em-up', 'sprites.enemy');
      if (enemySpriteUrl) {
        const cached = gameAssetCache.getElement(enemySpriteUrl);
        if (cached && cached instanceof HTMLImageElement) {
          // Use preloaded asset
          enemySpriteRef.current = cached;
        } else {
          // Load if not cached
  ...
  const spawnEnemy = useCallback(() => {
    ...
    if (!canvas) return;
      ...
      const yPos = Math.random() * (canvasHeight - ENEMY_SIZE);
    const speed = ENEMY_BASE_SPEED + Math.random() * 50;

    enemiesRef.current.push({
      id: `enemy-${Date.now()}-${Math.random()}`,
      x: canvasWidth + ENEMY_SIZE,
      y: yPos,
      width: ENEMY_SIZE,
      height: ENEMY_SIZE,
      speed,
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      // Clear canvas (use CSS dimensions, context is already scaled)
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (enemy.x + enemy.width < 0 || enemy.x > canvasWidth || enemy.y + enemy.height < 0 || enemy.y > canvasHeight) {
          enemies.splice(i, 1);
        }
      }
  ...
  // Canvas sizing is handled by sizeCanvasFromContainer in the game loop effect
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full min-h-[600px] aspect-[4/3]">
      ...
      <canvas
        ref={canvasRef}
        className="w-full h-full rounded-lg border border-pink-500/20 shadow-2xl"
