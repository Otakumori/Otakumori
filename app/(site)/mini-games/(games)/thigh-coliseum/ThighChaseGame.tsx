...
export default function ThighChaseGame({
  ...
  onStatsUpdate?: (stats: { score: number; lives: number; stage: number; progress?: number }) => void;
} = {}) {
  ...
  useEffect(() => {
    if (gameState === 'playing' && onStatsUpdate) {
      const progress = stage / 10; // Assuming 10 stages max, adjust as needed
      onStatsUpdate({
        score,
        lives,
        stage,
  ...
  }, [gameState, score, lives, stage, onStatsUpdate]);
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      setCameraX((prev) => {
        const targetX = Math.max(0, player.x - canvasWidth / 3); // Keep player in left third of screen
      ...
      setPlayer((prev) => {
        ...
        if (keysRef.current.has('d') || keysRef.current.has('arrowright')) {
          newX = Math.min(canvasWidth - 100, newX + speed);
        }
  ...
  useEffect(() => {
    ...
    // Clear canvas using design system background
    const bgColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-background').trim() || '#1b1a17'
      : '#1b1a17';
    ...
    // Draw background using design system colors
    const surfaceColor = typeof window !== 'undefined'
      ? getComputedStyle(document.documentElement).getPropertyValue('--color-surface-primary').trim() || '#413f4f'
      : '#413f4f';
    const bgGradient = ctx.createLinearGradient(0, 0, 0, canvasHeight);
    bgGradient.addColorStop(0, bgColor);
    bgGradient.addColorStop(0.5, surfaceColor);
    bgGradient.addColorStop(1, bgColor);
    ctx.fillStyle = bgGradient;
    ctx.fillRect(cameraX, 0, canvasWidth + 200, canvasHeight);

    // Moving pattern overlay
    ctx.fillStyle = '#2a2a2a';
    const bgOffset = (distance * 0.1) % 60;
    for (let x = -bgOffset + cameraX; x < canvasWidth + cameraX; x += 60) {
      ctx.fillRect(x, 0, 30, canvasHeight);
    }

    // Draw ground
    ctx.fillStyle = '#3a3a3a';
    ctx.fillRect(cameraX, GROUND_Y, canvasWidth + 200, canvasHeight - GROUND_Y);

    // Draw ground pattern
    ctx.fillStyle = '#4a4a4a';
    const groundOffset = (distance * 0.2) % 40;
    for (let x = -groundOffset + cameraX; x < canvasWidth + cameraX; x += 40) {
    ...
    if (player.speedBoost > 0) {
      ...
      ctx.fillText(`Speed Boost: ${Math.ceil(player.speedBoost / 1000)}s`, canvasWidth - 200, 30);
    }
    if (player.shielded > 0) {
      ctx.fillStyle = '#00ffff';
      ctx.fillText(`Shield: ${Math.ceil(player.shielded / 1000)}s`, canvasWidth - 200, 55);
    }
    ...
    if (pursuerDistance < 200) {
      ...
      ctx.fillText(`DANGER: ${Math.floor(pursuerDistance)}px`, canvasWidth - 200, 80);
    }
  });
  ...
  // Game Over Screen removed - handled by GameStateMachine wrapper
  if (gameState === 'gameOver') {
    return null;
  }

  return (
    <div ref={containerRef} className="w-full h-full flex flex-col bg-black aspect-[4/3]">
      <canvas
        ref={canvasRef}
        className="w-full h-full bg-black border border-purple-500/30"
        style={{
          backgroundColor: 'var(--color-background, #1b1a17)',
        }}
      />
    </div>
  );
