...
class GameEngine {
  ...
  constructor(canvas: HTMLCanvasElement, skin: string, mode: string) {
    ...
    // Get design dimensions (CSS size, not actual canvas size)
    const designWidth = parseInt(canvas.style.width) || 960;
    const designHeight = parseInt(canvas.style.height) || 540;

    // Initialize game objects
    this.ragdoll = new Ragdoll(designWidth / 2, 60);
  ...
  private spawnInitialBubbles() {
    // Get design dimensions (CSS size, not actual canvas size)
    const designWidth = parseInt(this.canvas.style.width) || 960;
  ...
  update(deltaTime: number) {
    ...
    // Get design dimensions for cleanup
    const designHeight = parseInt(this.canvas.style.height) || 540;

    // Clean up off-screen bubbles
    this.bubbles = this.bubbles.filter(
      (bubble) => bubble.y > -bubble.radius && bubble.y < designHeight + bubble.radius,
    );
  }
  ...
  render() {
    ...
    gradient.addColorStop(1, `${bgColor}1A`);
  ...
  private drawFanIndicators() {
    ...
    this.ctx.fillRect(0, 0, 100, designHeight);

    // Right fan
    this.ctx.fillRect(designWidth - 100, 0, 100, designHeight);
  ...
  reset() {
    ...
    this.score = 0;
    this.gameTime = 0;
    this.ragdoll.reset(designWidth / 2, 60);
    this.bubbles = [];
    this.spawnInitialBubbles();
  }
