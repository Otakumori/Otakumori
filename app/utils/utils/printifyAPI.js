...
const handleApiError = (error) => {
  if (error.response) {
    ...
    logger.error('Printify API Request Error:', error.request);
    throw new Error('No response received from Printify API');
  } else {
    // Something happened in setting up the request that triggered an Error
    logger.error('Printify API Setup Error:', error.message);
...
export const fetchProductDetails = async (productId) => {
  try {
    ...
    logger.error(`Failed to fetch product details for ${productId}:`, error);
    throw error;
  }
};
...
export const publishProduct = async (productId) => {
  try {
    ...
    logger.error(`Failed to publish product ${productId}:`, error);
    throw error;
  }
};
