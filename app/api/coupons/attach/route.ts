...
export async function POST(req: NextRequest) {
  const rl = await rateLimit(req, { windowMs: 60_000, maxRequests: 20, keyPrefix: 'coupons:attach' });
  ...
  try {
    ...
    const coupons = await prisma.coupon.findMany({ where: { code: { in: norm } }, select: { id: true, code: true } });
    ...
    for (const code of norm) {
      ...
      if (!cid) { results.push({ code, attached: false }); continue; }
      try {
        await prisma.couponRedemption.upsert({
          where: { couponId_clientReferenceId: { couponId: cid, clientReferenceId } },
          update: { userId: userId ?? undefined, status: 'PENDING' },
          create: { couponId: cid, clientReferenceId, userId: userId ?? undefined, status: 'PENDING' },
        });
    ...
    logger.error('coupons_attach_error', { route: '/api/coupons/attach' }, { error: String(e?.message || e) });
