...
export async function GET(){
  ...
  const posts = await prisma.soapstoneMessage.findMany({
    ...
    take: 50,
    select: { id:true, content:true, createdAt:true, upvotes:true, isHidden:true, isFlagged:true }
  });

  // Owned cosmetics (for badges/titles we infer from SKU)
  const inv = await prisma.inventoryItem.findMany({ where:{ userId: u.id }, orderBy:{ acquiredAt: "desc" } });
  ...
  const daily = { used: u.dailyClicks, limit: Number(process.env.NEXT_PUBLIC_DAILY_PETAL_LIMIT ?? 500) };
