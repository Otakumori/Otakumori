...
export async function GET() {
  ...
  const progress = await db.questProgress.findMany({ where: { userId, date: today } }).catch(() => []);
  const claimed = await db.questClaim.findMany({ where: { userId, date: today } }).catch(() => []);

  const progressMap: Record<string, number> = Object.fromEntries(progress.map((p: { questId: string; count: number }) => [p.questId, p.count]));
...
export async function POST(req: Request) {
  ...
  const prog: { count: number } | null = await db.questProgress.findUnique({ where: { userId_date_questId: { userId, date: today, questId } }}).catch(() => null);
  const count = prog?.count ?? 0;

  if (count < q.target) return new NextResponse("Not complete", { status: 400 });

  const already: { id: string } | null = await db.questClaim.findUnique({ where: { userId_date_questId: { userId, date: today, questId } }}).catch(() => null);
