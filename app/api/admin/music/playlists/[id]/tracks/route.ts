...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!title || !url) return NextResponse.json({ ok: false, error: "Missing fields" }, { status: 400 });
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!Array.isArray(order)) return NextResponse.json({ ok: false, error: "Invalid order format" }, { status: 400 });
