...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logger.error('admin_petalshop_upsert_error', upsertCtx, {
      error: String(error instanceof Error ? error.message : error),
    });
    return NextResponse.json(
      problem(500, 'upsert_failed', error instanceof Error ? error.message : undefined),
      {
        status: 500,
      },
    );
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    logger.error('admin_petalshop_delete_error', deleteCtx, {
      error: String(error instanceof Error ? error.message : error),
    });
    return NextResponse.json(
      problem(500, 'delete_failed', error instanceof Error ? error.message : undefined),
      {
        status: 500,
      },
    );
