...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!printifyResponse.ok) {
      throw new Error(`Printify API error: ${printifyResponse.status} ${printifyResponse.statusText}`);
    }
    ...
    const products = printifyData.data.map((product: any) => ({
      ...
      category: product.tags.find((tag: string) => ['apparel', 'accessories', 'home-decor'].includes(tag)) || 'other',
      tags: product.tags,
      metadata: {
    ...
    // Cache for 15 minutes (Printify data doesn't change frequently)
