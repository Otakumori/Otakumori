...
export async function POST(request: NextRequest) {
  try {
    ...
    if (idempotencyKey) {
      ...
      if (existingFinish) {
        return NextResponse.json(
          { ok: false, error: 'Duplicate finish request' },
          { status: 409 }
        );
      }
    }
    ...
    if (!gameRun) {
      return NextResponse.json(
        { ok: false, error: 'Game run not found' },
        { status: 404 }
      );
    }
    ...
    if (gameRun.finishedAt) {
      return NextResponse.json(
        { ok: false, error: 'Game run already finished' },
        { status: 409 }
      );
    }
    ...
    if (runDuration < minDuration || runDuration > maxDuration) {
      return NextResponse.json(
        { ok: false, error: 'Invalid run duration' },
        { status: 400 }
      );
    }
...
async function checkAchievements(userId: string, gameKey: string, score: number, meta: any): Promise<string[]> {
...
async function unlockAchievement(userId: string, achievementCode: string, unlockedList: string[]): Promise<void> {
  try {
