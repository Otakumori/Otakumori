...
export async function GET(req: Request){
  ...
  if (tag) where.eventTag = tag;
    ...
    where, orderBy: [{ visibleFrom: "asc" }, { name: "asc" }], take: 80
  });

  let inventory:any[] = [], balances = { petals:0, runes:0 };
  if (userId){
    ...
    inventory = inv.map(x=>({
      sku: x.sku, kind: x.kind, name: x.sku, previewUrl: previewFromSku(x.sku)
    }));
  }
