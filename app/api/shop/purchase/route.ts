...
export async function POST(req: Request){
  ...
  if (item.visibleFrom && item.visibleFrom > new Date()) return NextResponse.json({ ok:false, error:"not_visible" },{ status:403 });
  if (item.visibleTo && item.visibleTo < new Date()) return NextResponse.json({ ok:false, error:"expired" },{ status:403 });

  const res = await prisma.$transaction(async(tx)=>{
    if (needRunes) await tx.user.update({ where:{ id:user.id }, data:{ runes: { decrement: needRunes } } });
    if (needPetals) await tx.user.update({ where:{ id:user.id }, data:{ petalBalance: { decrement: needPetals } } });
