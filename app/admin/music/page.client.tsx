    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-zinc-800/60 p-4">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={createPl} className="rounded-md bg-emerald-600 px-3 py-1.5 hover:bg-emerald-500">
            Create
          </button>
        </div>
      </div>
      ...
      {pls.map((pl) => (
        <div key={pl.id} className="rounded-2xl border border-zinc-800/60 p-4">
          <div className="mb-2 flex items-center justify-between">
            <div className="font-medium">{pl.name} {pl.isPublic ? "• Public" : "• Private"}</div>
            <button
          ...
          <TrackUploader onUpload={(file, meta) => addTrack(pl.id, file, meta.title, meta.artist)} />
          <ul className="mt-3 space-y-2">
            {pl.tracks.map(t => (
              <li key={t.id} className="flex items-center justify-between rounded-md border border-zinc-800/60 px-3 py-2">
                <div>
...
function TrackUploader({ onUpload }: { onUpload: (file: File, meta: { title: string; artist: string }) => void }) {
