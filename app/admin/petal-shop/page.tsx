    ...
    <AdminLayout>
      <div className="p-6">
        ...
        <div className="mb-4 flex items-center gap-2">
          <button
            onClick={() =>
              setEditing({ sku: '', name: '', kind: 'COSMETIC', pricePetals: 0, metadata: {} })
            }
            className="rounded bg-pink-600 px-4 py-2 text-white"
          >
            New Item
          </button>
          <button onClick={load} className="rounded border border-white/20 px-3 py-2 text-white/80">
            Refresh
          </button>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-white/90">
                    ...
                    <div className="text-xs text-neutral-400">
                      {it.kind} • {it.pricePetals ?? 0} petals
                    </div>
                  </div>
            ...
            <div className="grid gap-3">
              ...
              <label className="block text-sm text-neutral-300">
                ...
                <select
                  ...
                  className="mt-1 w-full rounded bg-black/40 px-2 py-1 text-white outline-none ring-1 ring-white/15"
                >
                  {['COSMETIC', 'OVERLAY', 'TEXT', 'CURSOR', 'COUPON'].map((k) => (
              ...
              <label className="block text-sm text-neutral-300">
                Event Tag
                <input
                  value={editing.eventTag ?? ''}
                  onChange={(e) => setEditing({ ...editing, eventTag: e.target.value })}
                  className="mt-1 w-full rounded bg-black/40 px-2 py-1 text-white outline-none ring-1 ring-white/15"
                />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm text-neutral-300">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditing({ ...editing, visibleFrom: e.target.value })}
                    className="mt-1 w-full rounded bg-black/40 px-2 py-1 text-white outline-none ring-1 ring-white/15"
                  />
                </label>
                <label className="block text-sm text-neutral-300">
              ...
              <label className="block text-sm text-neutral-300">
                Preview URL
                <input
                  value={editing.metadata?.previewUrl ?? ''}
                  onChange={(e) =>
                    setEditing({
                      ...editing,
                      metadata: { ...(editing.metadata || {}), previewUrl: e.target.value },
                    })
                  }
                  className="mt-1 w-full rounded bg-black/40 px-2 py-1 text-white outline-none ring-1 ring-white/15"
                />
              ...
              <label className="block text-sm text-neutral-300">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditing({
                      ...editing,
                      metadata: { ...(editing.metadata || {}), description: e.target.value },
                    })
                  }
                  className="mt-1 w-full min-h-[80px] rounded bg-black/40 px-2 py-1 text-white outline-none ring-1 ring-white/15"
                />
              </label>
              {editing.kind === 'COUPON' && (
              ...
              <div className="flex items-center gap-2">
                <button onClick={onSave} className="rounded bg-pink-600 px-4 py-2 text-white">
                  Save
                </button>
                <button
                  onClick={() => setEditing(null)}
                  className="rounded border border-white/20 px-4 py-2 text-white/80"
                >
                  Cancel
                </button>
              </div>
