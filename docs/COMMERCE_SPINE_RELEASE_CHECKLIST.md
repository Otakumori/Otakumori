...
- Current validation commit: `04a61aec`
- Release-validation automation commit: `ef86ef7f`
- Products API compatibility commits: `fabb1cab`, `e3025573`, `04a61aec`
- Current Ready Preview URL: `https://otaku-mori-kov6p2s2y-otaku-mori-babe.vercel.app`
- Vercel build status: Ready.
- Local gates after the products compatibility patches: `pnpm type-check` passed, `pnpm lint` passed with 0 errors and 198 warnings under the configured threshold, `pnpm prisma generate` passed, and `pnpm build` passed.
...
- Exact smoke target: `BASE_URL=https://otaku-mori-kov6p2s2y-otaku-mori-babe.vercel.app pnpm smoke`. Do not record the bypass secret.
...
- Products API status: fixed for Preview route stability. `BASE_URL=https://otaku-mori-kov6p2s2y-otaku-mori-babe.vercel.app SMOKE_ROUTES=/api/v1/products?limit=25&inStock=true pnpm smoke` returned 200. The response is structured and non-crashing, but the current Preview catalog has `productCount=0`, `variantCount=0`, and `sellableVariantCount=0`, so checkout seeding is still blocked by catalog data/sync rather than by an API 500.
- Clerk Preview E2E status: blocked by Preview-domain Clerk CORS/dashboard config. `BASE_URL=https://otaku-mori-kov6p2s2y-otaku-mori-babe.vercel.app pnpm test:clerk-preview` reaches the Preview with bypass, but the deployed production-oriented Clerk key/config blocks the Vercel Preview origin from loading ClerkJS/Frontend API requests for the custom domain (`otaku-mori.com`). Do not hack around this in app code; rerun the Clerk test after merge/promotion on the real custom domain or configure dedicated Preview Clerk origins.
- Commerce Playwright status: partially automated, not release-green. `BASE_URL=https://otaku-mori-kov6p2s2y-otaku-mori-babe.vercel.app pnpm test:commerce-release` passed static checks, route smoke, health/provider diagnostics, and desktop/mobile/WebKit route rendering. It failed only the signed-out cart/checkout seeding checks because the Preview catalog has no sellable variants, and skipped signed-in checkout because Clerk Preview CORS is a known Preview-domain blocker. Do not call PR #29 release-ready until catalog data, auth, Stripe, provider, and mobile manual gates are proven.
