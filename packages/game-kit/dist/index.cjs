...
function pollKeyboard(system, state) {
  ...
  state["Jump" /* Jump */] = keyboard["Jump" /* Jump */].some(
    (key) => system.keyPressed.has(key)
  );
  state["Attack" /* Attack */] = keyboard["Attack" /* Attack */].some(
    (key) => system.keyPressed.has(key)
  );
  state["Dash" /* Dash */] = keyboard["Dash" /* Dash */].some(
    (key) => system.keyPressed.has(key)
  );
  state["Pause" /* Pause */] = keyboard["Pause" /* Pause */].some(
    (key) => system.keyPressed.has(key)
  );
}
function pollGamepad(system, state) {
  ...
  state["Jump" /* Jump */] = state["Jump" /* Jump */] || (gamepad.buttons[jumpButton]?.pressed ?? false);
  const attackButton = mapping["Attack" /* Attack */].button;
  state["Attack" /* Attack */] = state["Attack" /* Attack */] || (gamepad.buttons[attackButton]?.pressed ?? false);
  const dashButton = mapping["Dash" /* Dash */].button;
  state["Dash" /* Dash */] = state["Dash" /* Dash */] || (gamepad.buttons[dashButton]?.pressed ?? false);
  const pauseButton = mapping["Pause" /* Pause */].button;
  state["Pause" /* Pause */] = state["Pause" /* Pause */] || (gamepad.buttons[pauseButton]?.pressed ?? false);
}
...
function justPressed(system, action) {
  ...
  return mapping.some(
    (key) => system.keyPressed.has(key) && !system.prevKeyPressed.has(key)
  );
}
...
var DEFAULT_TRANSITIONS = [
  // Idle <-> Walk
  {
    from: "Idle" /* Idle */,
    to: "Walk" /* Walk */,
    condition: (data) => data.horizontalSpeed > ANIM_THRESHOLDS.IDLE_SPEED && data.horizontalSpeed < ANIM_THRESHOLDS.WALK_SPEED && data.isGrounded,
    duration: 0.2
  },
  {
    from: "Walk" /* Walk */,
  ...
  },
  {
    from: "Run" /* Run */,
    to: "Walk" /* Walk */,
    condition: (data) => data.horizontalSpeed < ANIM_THRESHOLDS.WALK_SPEED && data.horizontalSpeed > ANIM_THRESHOLDS.IDLE_SPEED && data.isGrounded,
    duration: 0.15
  },
  ...
  },
  {
    from: "Land" /* Land */,
    to: "Walk" /* Walk */,
    condition: (data) => data.isGrounded && data.horizontalSpeed > ANIM_THRESHOLDS.IDLE_SPEED && data.horizontalSpeed < ANIM_THRESHOLDS.WALK_SPEED,
    duration: 0.2
  },
  {
    from: "Land" /* Land */,
  ...
  },
  {
    from: "Attack" /* Attack */,
    to: "Walk" /* Walk */,
    condition: (data) => !data.attackRequested && data.horizontalSpeed > ANIM_THRESHOLDS.IDLE_SPEED && data.horizontalSpeed < ANIM_THRESHOLDS.WALK_SPEED,
    duration: 0.2
  },
  {
    from: "Attack" /* Attack */,
...
function updateHFSM(hfsm, data, dt, transitions = DEFAULT_TRANSITIONS) {
  ...
  if (hfsm.transitionProgress < 1) {
    hfsm.transitionProgress = Math.min(
      1,
      hfsm.transitionProgress + dt / hfsm.transitionDuration
    );
  }
...
function updateBlendWeights(hfsm, data) {
  ...
  if (hfsm.transitionProgress < 1) {
    ...
    if (hfsm.current === "Idle" /* Idle */ || hfsm.current === "Walk" /* Walk */ || hfsm.current === "Run" /* Run */) {
